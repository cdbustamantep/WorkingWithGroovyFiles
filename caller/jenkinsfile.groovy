...
def repoclone(){ //defines a function to clone the repositories to the Jenkins' workspace
    checkout([$class: 'GitSCM', branches: [[name: '*/'+'main']], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'RelativeTargetDirectory'], [$class: 'MessageExclusion', excludeMessage: '(?s).*JENKINS_IGNORE.*']], submoduleCfg: [], userRemoteConfigs: [[url:"https://github.com/cdbustamantep/WorkingWithGroovyFiles.git"]]])
    }
